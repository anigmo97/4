...
CSupport*       Support_tras_level_parser; //AÑADIDO PARTE ENTREGA3
...
UGKS_String CLeP_Tags[MAXTAGS_L - MAXTAGS_D] =
{
	...
	"NUMLINES",
	"PERIODE",
	"PERIODO_ESPERA_CURACIONES", //AÑADIDO PARTE ENTREGA3
	...
	"SUPPORT_COLOR", //AÑADIDO PARTE ENTREGA3
	...
	"UMBRAL_PASA_A_ESTADO_DAÑADO", //AÑADIDO PARTE3
	"UMBRAL_DAÑO_PLAYER_CURAR", //AÑADIDO PARTE3
	"VERSION",
	"X",
	"Y",
	"Z",
...
void CLevelReader::StartTag(CLiteHTMLTag *pTag, DWORD dwAppData, bool &bAbort)
{
	...
	case CHARACTER_L:
					...
					case SUPPORT_COLOR_L: //AÑADADIO ENTREGA3
					case SUPPORT_NUM_CURATIONS_L: //AÑADIDO ENTREGA3
					case UMBRAL_DAÑO_PLAYER_CURAR_L: //AÑADIDO ENTREGA3 
					case UMBRAL_PASA_A_ESTADO_DAÑADO_L:  //AÑADIDO ENTREGA3
					case PERIODO_ESPERA_CURACIONES_L: //AÑADIDO ENTREGA3
					case ACCELERATION_L:
					case ANIMATION2D_L:
					case BOUNCE_L:
					case EXPLOSION_L:
...
void CLevelReader::EndTag(CLiteHTMLTag *pTag, DWORD dwAppData, bool &bAbort)
{
	...
	switch(Top())
	{
	 ...
	 case SUBTYPE_L:
	 case SUPPORT_COLOR_L: //AÑADIDO ENTREGA 3
	 case SUPPORT_NUM_CURATIONS_L: //AÑADIDO ENTREGA3
	 case TEXTURE_L:
	 case TEXTURE2D_L:
	 case TEXTURE3D_L:
...
if (bAbort) return;
	...
	switch(Top())
	{
	case UMBRAL_PASA_A_ESTADO_DAÑADO_L: //AÑADIDO PARTE ENTREGA3
		switch (CLeP_CharType)
		{
		case CHARS_SUPPORT:
			Support->umbral_pasa_a_dañado = atof(UGKS_string2charstr(rText));
			break;
		}
		break;
	case UMBRAL_DAÑO_PLAYER_CURAR_L: //AÑADIDO PARTE ENTREGA3
		switch (CLeP_CharType)
		{
		case CHARS_SUPPORT:
			Support->umbral_daño_player = atof(UGKS_string2charstr(rText));
			break;
		}
		break;
	case PERIODO_ESPERA_CURACIONES_L: //AÑADIDO PARTE ENTREGA3
		switch (CLeP_CharType)
		{
		case CHARS_SUPPORT:
			Support->tiempo_entre_curaciones = atof(UGKS_string2charstr(rText));
			break;
		}
		break;
	case SUPPORT_NUM_CURATIONS_L: //AÑADIDO PARTE ENTREGA3
		switch (CLeP_CharType)
		{
		case CHARS_SUPPORT:
			Support->num_curaciones = atof(UGKS_string2charstr(rText));
			break;
		}
		break;
	case SUPPORT_COLOR_L: //AÑADIDO PARTE3
		switch (CLeP_CharType)
		{
		case CHARS_SUPPORT:
			if (rText == "ROJO" || rText == "rojo") {
				Support->color = SUPPORT_COLOR_ROJO;
				Support->cambiaMesh(SUPPORT_VIVO);
			}else if (rText == "GRIS" || rText == "gris") {
				Support->color = SUPPORT_COLOR_GRIS;
				Support->cambiaMesh(SUPPORT_VIVO);
			}else if (rText == "AMARILLO" || rText == "amarillo") {
				Support->color = SUPPORT_COLOR_AMARILLO;
				Support->cambiaMesh(SUPPORT_VIVO);
			}
			break;
		}
		break;
	case PLAYER_CURATION_L: // AÑADIDO PARTE2
		switch (CLeP_CharType)
		{
		case CHARS_SUPPORT:
			Support->player_curation = atof(UGKS_string2charstr(rText));
			break;
		}
		break;
	case ESCUDO_L: // AÑADIDO PARTE 2
		switch (CLeP_CharType)
		{
		case CHARS_SUPPORT:
			Support->escudo = atof(UGKS_string2charstr(rText));
			break;
		}
		break;
	case ALTURA_L: // AÑADIDO PARTE 2
		switch (CLeP_CharType)
		{
		case CHARS_SUPPORT:
			Support->altura = atof(UGKS_string2charstr(rText));
			break;
		}
		break;
	case FROZEN_TIME_AFTER_IMPACT_L: // AÑADIDO PARTE 2
		switch (CLeP_CharType)
		{
		case CHARS_SUPPORT:
			Support->frozen_time_after_impact = atof(UGKS_string2charstr(rText));
			break;
		}
		break;
	...
	case MESH_L:
			switch (CLeP_CharType)
			{
				...
				case CHARS_SUPPORT: //AÑADIDO PARTE ENTREGA2
					msg = rText;
					// si hay color fijado, se usa la mesh predefinida, no se sobreescribe.
					if (Support->color!=SUPPORT_COLOR_AMARILLO && Support->color != SUPPORT_COLOR_GRIS && Support->color != SUPPORT_COLOR_ROJO) { 
...
void CLevelReader::EndParse(DWORD dwAppData, bool bIsAborted)
{
	...
	Support_tras_level_parser = (CSupport*)CharacterPool->get(CHARS_SUPPORT, UGKOBJM_NO_SUBTYPE);
	*Support_tras_level_parser = *Support; // me guardo todos los valores tras el parseado ENTREGA3
	UNUSED_ALWAYS(dwAppData);
