...
def greedy_knapsack(w, v, W):
    ...
    ###########################################################################################
    ###########################################################################################
    # creo una lista de tuplas de la forma ( indice, peso, valor)
    lista_tuplas = [ (i,w[i],v[i]) for i in range(0,len(w))]
    # creo una lista con las tuplas ordenadas por valor/peso de mayor a menor
    lista_tuplas_ordenada = sorted(lista_tuplas, key=lambda x: x[2]/x[1],reverse=True)

    for indice,peso,valor in lista_tuplas_ordenada:
        if peso < W:
            x[indice] = 1
            W -= peso
            profit += valor
