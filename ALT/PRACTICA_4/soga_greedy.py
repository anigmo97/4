...
def greedy_tug_of_war(v):
    ...
    ###########################################################################################
    ###########################################################################################
    # IMPORTANTE histograma[ratio] += 1 ERROR --> LAS DOS PARTES NO MIDEN LO MISMO
    v1 = sorted(v)
    left,right =[],[]
    accumleft,accumright = 0,0

    # version cogiendo el primer y el ultimo elemento obtuvo 9%
    # la mejor opcion es coger de dos en dos y darle el mayor al que menor suma tenga           
    print("\n\nVector ordenado inicial = {}".format(v1))
    while len(v1) > 0 :
        if sum(left)>sum(right): # SE PUEDE HACER MUCHO MAS EFICIENTE GUARDANDO LA SUMA EN CADA ITERACION
            left.append(v1.pop(0))
            right.append(v1.pop(0))
        else:
            right.append(v1.pop(0))
            left.append(v1.pop(0)) 
        
    print("\nVector ordenado final = {}".format(v1))
    print("{}\n{}".format(right,left))

    ## COMPROBAR SI LA SOLUCION CUMPLE LOS REQUISITOS
    if len(right) != len(left):
        raise Exception('\n\nRight y left deben tener el mismo numero de elementos')
    elif len(right) + len(left) != N2:
        raise Exception('\n\nLa suma del numero de elementos de right y left no conincide con el numero de elementos totales N2')
