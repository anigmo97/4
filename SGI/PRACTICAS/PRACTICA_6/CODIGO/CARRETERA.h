...
#define WITH_RAYA_CENTRAL_CARRETERA 0.2
...
float calcula_dim_vector_para_punto_interior(float punto, VECTORES_QUAD vec, DIMENSION dim) {
	...
	case VECTOR_0:
		if (dim == X_DIM) {
			return punto - vector_n[0] * WITH_RAYA_CENTRAL_CARRETERA / 2;
		}
		else if (dim == Z_DIM) {
			return punto - vector_n[1] * WITH_RAYA_CENTRAL_CARRETERA / 2;
		}
		break;
	case VECTOR_1:
		if (dim == X_DIM) {
...
void dibuja_carretera(PARTE_CARRETERA p) {
	...
	{
	case QUAD_ARRIBA:
		quad(v3_interior, v2_interior, v2, v3, 10, 10);
		break;
	case QUAD_ABAJO:
		quad(v0, v1, v1_interior, v0_interior, 10, 10);
		break;
	case RAYA_CENTRAL_AMARILLA:
		glColor3f(0.5, 0.5, 0);
		quad(v0_interior, v1_interior, v2_interior, v3_interior, 10, 10);
		glColor3f(0, 0, 0);
		break;
	default:
		break;
