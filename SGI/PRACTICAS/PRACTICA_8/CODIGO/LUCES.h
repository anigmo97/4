...
/*POSICIONES FAROLAS*/
int puntos_en_x_luces[4];
float puntos_en_z_luces[4];
...
void inicializa_Luces(void) {
	...
	GLfloat Al0[] = { 0.1,0.1,0.1,0.1 }; // Color ambiental de la luz
...
void coloca_luces_farolas(int x_inicial_1) {
	...
	for (auto i = 1; i < 5; i++) {
		puntos_en_x_luces[i - 1] = (x_inicial_1 + separacion_farolas * i);
		puntos_en_z_luces[i - 1] = funcion_f(puntos_en_x_luces[i - 1]);
	}
	/*for (auto i = 0; i < 4; i++) {
